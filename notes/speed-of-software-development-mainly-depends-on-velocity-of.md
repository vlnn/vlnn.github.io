...
Compiling the binary is not learning (even though debugging failing compiling is), so it is not something to improve in this approach. Typing is not learning, and so it is negligible in this concept (even though people, of course, type a lot, this is not what takes their time, unless they have some physical conditions).

Related to:

-  [Pair programming costs x2](pair-programming-costs-x2.md), but [works great for juniors](pair-programming-works-best-for-cognitive-demanding-tasks.md) which may be related to the topic as they learn faster? (and seniors don't need to learn that much, so bang for the buck is smaller)
