---
title: Open mode of operating
date: 2026-08-28
tags: [learning, methods, psy]
brain-id: 8a566118-c72f-4da7-8f87-1db403fbdb7d
---

Receipt of open mode:

1. Space (or [safe environment](safe-environment.md) without disturbance)
2. Time to get in (\~30 minutes to get in, and \~1h to actually work)
(1-2) are [qualities of any play](play-is-distinct-from-ordinary-life-both-as-to-locality-and.md) or [zazen](zazen.md)
1. Time usage (a longer discomfort to bare with)
2. Confidence ([safe environment](safe-environment.md) again)
3. [Humor](humor.md) as a driver from [close](closed-mode-of-operating.md) to open mode

## Related

- [Humor](humor.md)
- [Closed Mode of Operating](closed-mode-of-operating.md)
- [Play is distinct from ordinary life both as to locality and duration](play-is-distinct-from-ordinary-life-both-as-to-locality-and.md)

